...
namespace :db do
  ...
  ENV['DATABASE'] = 'sin_dev' if environment == 'development'
  ENV['DATABASE'] = 'sin_test' if environment == 'test'
  ENV['DATABASE'] = 'sin_prod' if environment == 'production'
  # connection_string = ENV['DATABASE_URL'] || ENV["DATABASE_URL_#{environment.upcase}"]
  migrations_directory = 'db/migrations'
  connection_string = "postgres://nik:password@localhost/#{ENV['DATABASE']}"
  ...
  task :seed do
    ...
    #раньше были в my-sinatra.rb:28
